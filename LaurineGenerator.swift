...
class Localization {
    ...
    func writerWithSwiftImplementation() -> StreamWriter {
        ...
        writer.writeMarkWithName(name: "Extensions")
...
class StreamWriter {
    ...
    func writeRequiredExtensions() {
        ...
        self.store(string: "private extension String {\n")
        self.store(string: "\n")
        self.store(string: "    var localized: String {")
        self.store(string: " return  \(BASE_CLASS_NAME).bundle.localizedString(forKey: self, value: nil, table: nil) ")
        self.store(string: "}\n")
        self.store(string: "\n")
        self.store(string: "    func localizedWithComment(comment:String) -> String {")
        self.store(string: " return  \(BASE_CLASS_NAME).bundle.localizedString(forKey: self, value: nil, table: nil) ")
        self.store(string: "}\n")
        self.store(string: "}\n")
    }
...
//        let tableName = table != nil ? "\"\(table!)\"" : "nil"
        return TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "/// Base translation: \(baseTranslation)\n"
            + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "public static var \(name) : String { return  \"\(key)\".localized }\n"//"public static var \(name) : String = NSLocalizedString(\"\(key)\", tableName: \(tableName), bundle: _shared.bundle, value: \"\", comment: \"\")\n"
    }
...
//        let tableName = table != nil ? "\"\(table!)\"" : "nil"
        return TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "/// Base translation: \(baseTranslation)\n"
            ...
            + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel + 1) + "return String(format: \(BASE_CLASS_NAME).bundle.localizedString(forKey: \"\(key)\", value: nil, table: nil), \(params))\n"//"return String(format: NSLocalizedString(\"\(key)\", tableName: \(tableName), bundle: _shared.bundle, value: \"\", comment: \"\"), \(params))\n"
            + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "}\n"
    }
